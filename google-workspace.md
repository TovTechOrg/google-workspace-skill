...
- **Helper script:** `C:\Users\meira\.claude\gdrive.py`
- **Token file:** `C:\Users\meira\.claude\drive_token.json` (auto-refreshes, no re-auth needed)
- **Client secret:** `C:\Users\meira\.claude\client secret.json`
- **Scopes:** `drive` (full) + `gmail.modify` + `gmail.compose` + `gmail.readonly`

All operations run via:
```bash
py "C:\Users\meira\.claude\gdrive.py" <cmd> [args]
```
...
### List recent emails
```bash
py "C:\Users\meira\.claude\gdrive.py" emails [N]
```
Returns: message ID, threadId, From, Subject, Date, snippet. Default N=5.

### Search emails (Gmail query syntax)
```bash
py "C:\Users\meira\.claude\gdrive.py" emails_search "<query>" [N]
```
Gmail query examples: `from:someone@example.com`, `subject:invoice`, `after:2026/05/01`, `is:unread`, `has:attachment`

### Read full email
```bash
py "C:\Users\meira\.claude\gdrive.py" email <message_id>
```

### Read full email thread
```bash
py "C:\Users\meira\.claude\gdrive.py" thread <thread_id>
```
Returns all messages in the thread in order.

### Send email
```bash
py "C:\Users\meira\.claude\gdrive.py" send "<to@email.com>" "<Subject>" "<body text>"
```
For multi-line body, use Python directly:
```bash
py -c "
import sys; sys.path.insert(0, r'C:\Users\meira\.claude')
import gdrive
...
### Reply to an email
```bash
py "C:\Users\meira\.claude\gdrive.py" reply <message_id> <body text>
```
Automatically uses the correct thread, subject (Re: ...), and recipient.

### Trash an email
```bash
py "C:\Users\meira\.claude\gdrive.py" trash_email <message_id>
```

### Mark email read / unread
```bash
py "C:\Users\meira\.claude\gdrive.py" mark_read <message_id>
py "C:\Users\meira\.claude\gdrive.py" mark_unread <message_id>
```
...
```bash
py "C:\Users\meira\.claude\gdrive.py" list <folder_id>
```
...
```bash
py "C:\Users\meira\.claude\gdrive.py" search "<query terms>"
```

### Get file metadata
```bash
py "C:\Users\meira\.claude\gdrive.py" info <file_id>
```
Returns: id, name, mimeType, modifiedTime, createdTime, size, parents, webViewLink.

### Create a folder
```bash
py "C:\Users\meira\.claude\gdrive.py" create_folder "<name>" [parent_folder_id]
```

### Create a new Google Doc
```bash
py "C:\Users\meira\.claude\gdrive.py" create_doc "<name>" [parent_folder_id]
```

### Upload a local file
```bash
py "C:\Users\meira\.claude\gdrive.py" upload "<local_path>" [parent_folder_id]
```
MIME type is auto-detected. Returns the new file's ID and link.

### Download a file
```bash
py "C:\Users\meira\.claude\gdrive.py" download <file_id> "<local_path>"
```
Google Docs → .txt, Sheets → .csv, Presentations → .pdf, others → as-is.

### Move a file
```bash
py "C:\Users\meira\.claude\gdrive.py" move <file_id> <new_parent_folder_id>
```

### Rename a file
```bash
py "C:\Users\meira\.claude\gdrive.py" rename <file_id> "<new name>"
```

### Trash a Drive file
```bash
py "C:\Users\meira\.claude\gdrive.py" trash_file <file_id>
```

### Share a file
```bash
py "C:\Users\meira\.claude\gdrive.py" share <file_id> <email> [role]
```
...
sys.path.insert(0, r'C:\Users\meira\.claude')
import gdrive
...
```bash
rm "C:/Users/meira/.claude/drive_token.json"
py "C:\Users\meira\.claude\gdrive.py" emails
```
...
```bash
py "C:\Users\meira\.claude\gdrive.py" emails 10
```

### "Find unread emails from X"
```bash
py "C:\Users\meira\.claude\gdrive.py" emails_search "from:x@example.com is:unread"
```

### "Show me the full conversation"
Get threadId from email, then:
```bash
py "C:\Users\meira\.claude\gdrive.py" thread <threadId>
```

### "Files created this week in folder X"
```bash
py "C:\Users\meira\.claude\gdrive.py" list_date <folder_id> 2026-05-12
```

### "What changed recently in my Drive?"
```bash
py "C:\Users\meira\.claude\gdrive.py" recent 20
```

### "Upload this file and share with someone"
```bash
py "C:\Users\meira\.claude\gdrive.py" upload "C:\path\to\file.pdf" <folder_id>
py "C:\Users\meira\.claude\gdrive.py" share <new_file_id> user@example.com writer
```
